...
This paper describes the design of \emph{HiRTOS} (High-Integrity RTOS),
...
in mind, and as such are written in C, an unsafe language. So, it would be desirable
to have an RTOS specifically designed for high-integrity applications, and written in a safer
language, even if application code is written in C/C++.
...
\bibitem{HiRTOS}
J. German Rivera, ``HiRTOS: a high-integrity multi-core RTOS kernel and separation kernel written in SPARK Ada'' \\
\url{https://github.com/jgrivera67/HiRTOS}


\bibitem{mutexAndCV}
Andrew Birrell et al, ``Synchronization primitives for a multiprocessor: a formal specification'',
